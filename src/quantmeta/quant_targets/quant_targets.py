...
def get_quad_reg1_path(*parts) -> Path:
    """Get path to package data files."""
    with resources.as_file(resources.files('quantmeta.data') / 'Regressions' / 'read_depth_variability' / 'Langenfeld_2025_0to10readsperbp.pkl') as p:
        base = Path(p).parent.parent.parent
    return base / 'data' / Path(*parts)

def get_quad_reg2_path(*parts) -> Path:
    """Get path to package data files."""
    with resources.as_file(resources.files('quantmeta.data') / 'Regressions' / 'read_depth_variability' / 'Langenfeld_2025_10to100readsperbp.pkl') as p:
        base = Path(p).parent.parent.parent
    return base / 'data' / Path(*parts)

def get_quad_reg3_path(*parts) -> Path:
    """Get path to package data files."""
    with resources.as_file(resources.files('quantmeta.data') / 'Regressions' / 'read_depth_variability' / 'Langenfeld_2025_100to1000readsperbp.pkl') as p:
        base = Path(p).parent.parent.parent
    return base / 'data' / Path(*parts)

def get_quad_reg4_path(*parts) -> Path:
    """Get path to package data files."""
    with resources.as_file(resources.files('quantmeta.data') / 'Regressions' / 'read_depth_variability' / 'Langenfeld_2025_gte1000readsperbp.pkl') as p:
        base = Path(p).parent.parent.parent
    return base / 'data' / Path(*parts)

def get_cutoff1_path(*parts) -> Path:
    """Get path to package data files."""
    with resources.as_file(resources.files('quantmeta.data') / 'Regressions' / 'threshold_read_depth_variability' / 'Langenfeld_2025_0to10readsperbp.pkl') as p:
        base = Path(p).parent.parent.parent
    return base / 'data' / Path(*parts)

def get_cutoff2_path(*parts) -> Path:
    """Get path to package data files."""
    with resources.as_file(resources.files('quantmeta.data') / 'Regressions' / 'threshold_read_depth_variability' / 'Langenfeld_2025_10to100readsperbp.pkl') as p:
        base = Path(p).parent.parent.parent
    return base / 'data' / Path(*parts)

def get_cutoff3_path(*parts) -> Path:
    """Get path to package data files."""
    with resources.as_file(resources.files('quantmeta.data') / 'Regressions' / 'threshold_read_depth_variability' / 'Langenfeld_2025_100to1000readsperbp.pkl') as p:
        base = Path(p).parent.parent.parent
    return base / 'data' / Path(*parts)

def get_cutoff4_path(*parts) -> Path:
    """Get path to package data files."""
    with resources.as_file(resources.files('quantmeta.data') / 'Regressions' / 'threshold_read_depth_variability' / 'Langenfeld_2025_gte1000readsperbp.pkl') as p:
        base = Path(p).parent.parent.parent
    return base / 'data' / Path(*parts)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cutoff_function1', required=True, help='Path to RMSE threshold function for 0-10 reads/bp')
    parser.add_argument('--cutoff_function2', required=True, help='Path to RMSE threshold function for 10-100 reads/bp')
    parser.add_argument('--cutoff_function3', required=True, help='Path to RMSE threshold function for 100-1000 reads/bp')
    parser.add_argument('--cutoff_function4', required=True, help='Path to RMSE threshold function for 1000+ reads/bp')
    ...
    output_path = Path(f'{out_dir}/Mapping/{sample_name}/{target_name}_mapping_analysis.txt')
    ...
    output_dir = Path(f'{out_dir}/Results/{sample_name}/{target_name}_concentrations.tsv')
    output_dir.parent.mkdir(parents=True, exist_ok=True)
