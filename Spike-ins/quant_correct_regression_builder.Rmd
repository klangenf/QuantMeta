...
Function to calculate R_G and number of gene copies per target sequence
```{r}
mapping_analysis <- function(sample_name, mapping_results, target_lengths, target_name) {
  ...
  mapping_targets_analysis = data.frame(cbind(targets$ID, targets$R_G, targets$gene_copies))
  colnames(mapping_targets_analysis) <- c("ID", "R_G", "total_avg_depth")
...
standards_w49bp$R_G <- as.numeric(standards_w49bp$R_G)
```
...
```{r}
# Update min_R_G function if applicable
min_R_G <- readRDS("Regressions/detection/Langenfeld_2022_R_G_detect")

standards_w49bp$detect_threshold <- predict.lm(min_R_G, newdata = standards_w49bp)
standards_w49bp <- subset(standards_w49bp, R_G > standards_w49bp$detect_threshold)
```
...
The terms in each equation may vary slightly. Based on Browne et al. (2020), Illumina technologies introduce as quadratic GC bias in sequencing results. Additionally, the total average read depth and R_G may also significantly impact the observed read depth variability. In Langenfeld et al. (2022), R_G was only a significant factor for the least abundant standards with incomplete coverage (i.e., 0-10 reads/bp bin).
```{r}
...
read_var_reg1 <- lm(log(avg_depth+1) ~ poly(avg_GC, 2, raw = TRUE) + poly(log(total_avg_depth), 2, raw = TRUE) + poly(R_G, 2, raw=TRUE), data = subset(standards_w49bp, total_avg_depth < 10))
