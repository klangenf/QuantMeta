...
Purpose: This R notebook is designed to develop a regression to determine the minimum R_G for confident detection with respect to a target's length. The regression from Langenfeld et al. (2022) may be adopted, but the minimum R_G is based on the standards proposed by FastViromeExplorer (Lithi et al. 2018) (10% read coverage and 0.3 observed/expected read distribution). If these parameters are deemed inappropriate for specific applications, it is recommended that new minimum R_G regressions be created for specific research needs.
...
mapping_analysis <- function(sample_name, mapping_results, target_lengths, target_name) {
  ...
  mapping_targets_logit = data.frame(cbind(targets$ID, targets$R_G, targets$LOD))
  colnames(mapping_targets_logit) <- c("ID", "R_G", "LOD")
  mapping_targets_analysis = data.frame(cbind(targets$ID, targets$R_G, targets$C_o, targets$R_FVE, targets$B_G, targets$gene_copies))
  colnames(mapping_targets_analysis) <- c("ID", "R_G", "C_o", "R_FVE", "B_G", "gene_copies")
...
for (i in 1:nrow(samples)) {
  ...
  colnames(all) <- c("ID", "R_G", "LOD", "downsample", "sample", "gene_copies")
...
for (i in 1:nrow(fail_samples)) {
  ...
  all <- cbind.data.frame(all, results$gene_copies)
  colnames(all) <- c("ID", "R_G", "LOD", "fail_set", "sample", "gene_copies")
...
```{r}
standards.lr <- glm(LOD ~ R_G, data = logit, family = binomial(link = "logit"))
```
...
logreg_assess <- function(test_data, sample_name) {
  ...
  cp <- cutpointr(test_data, R_G, above_LOD, method = maximize_boot_metric, metric = sum_sens_spec)
...
### Bin the database mapping results by the target length, then determine the optimal R_G cutpoint for each length bin to set as the optimal detection threshold
...
colnames(results_lengths) <- c("sample", "AUC", "R_G")
...
# save the linear model results to min_R_G
min_R_G <- lm(my.formula, results_lengths)

# save the linear model to the Regressions/detection directory
saveRDS(min_R_G, file = "Regressions/detection/detection_threshold")
```

### Update the Snakefile_quant_unknown and Snakefile_quantmeta files to include this min_R_G threshold
